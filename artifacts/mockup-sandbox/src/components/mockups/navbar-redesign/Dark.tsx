...
function ThemePill({ theme, onToggle }: { theme: "dark" | "light"; onToggle: () => void }) {
  return (
    <button
      ...
      className="relative flex items-center gap-0.5 p-1 rounded-full border bg-white/10 hover:bg-white/20 transition-all duration-200 shadow-sm border-white/15"
    >
      <span className={`flex items-center justify-center h-6 w-6 rounded-full transition-all duration-200 ${
        theme === "light"
          ? "bg-amber-400/20 text-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.4)]"
          : "text-white/30"
      }`}>
...
const NAV_ITEMS = [
  ...
  { icon: AtSign, label: "X Tools", color: "text-blue-400", bg: "bg-blue-500/10" },
  { icon: Code2, label: "Dev Tools", color: "text-orange-400", bg: "bg-orange-500/10" },
  { icon: TrendingUp, label: "SEO Tools", color: "text-pink-400", bg: "bg-pink-500/10" },
  { icon: Mail, label: "Email Tools", color: "text-cyan-400", bg: "bg-cyan-500/10" },
  { icon: Inbox, label: "Temp Mail", color: "text-teal-400", bg: "bg-teal-500/10" },
...
export function Dark() {
  ...
  return (
    <div className="min-h-screen font-['Inter']" style={{ background: "hsl(228 47% 8%)", color: "hsl(220 20% 92%)" }}>
      {/* Navbar */}
      <nav
        className="sticky top-0 z-50 border-b backdrop-blur-lg shadow-sm"
        style={{
          background: "hsla(228,47%,8%,0.85)",
          borderColor: "hsla(240,12%,13%,0.8)",
      ...
      >
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-12 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0 cursor-pointer">
            <div className="h-7 w-7 rounded-lg overflow-hidden shadow-lg" style={{ boxShadow: "0 0 12px rgba(124,58,237,0.4)" }}>
              <svg width="28" height="28" viewBox="0 0 180 180" fill="none">
                <defs>
                  ...
                  <linearGradient id="dFront" x1="35" y1="30" x2="145" y2="150" gradientUnits="userSpaceOnUse">
                ...
                <g stroke="url(#dFront)" strokeLinecap="round" fill="none">
                  <line x1="58" y1="44" x2="122" y2="136" strokeWidth="21"/><line x1="122" y1="44" x2="58" y2="136" strokeWidth="21"/>
                </g>
                <g stroke="url(#dFront)" strokeLinecap="square" fill="none">
                  <polyline points="46,38 31,38 31,142 46,142" strokeWidth="10"/><polyline points="134,38 149,38 149,142 134,142" strokeWidth="10"/>
                </g>
              </svg>
            </div>
            <span className="font-semibold text-sm tracking-tight" style={{ color: "hsl(220 20% 92%)" }}>X Toolkit</span>
            <span className="hidden lg:inline-flex text-[10px] font-medium px-1.5 py-0.5 rounded-full border"
              style={{ borderColor: "rgba(139,92,246,0.3)", color: "hsl(258 82% 66%)", background: "rgba(139,92,246,0.08)" }}>
              44+ Tools
            </span>
          </div>
          ...
          <div className="hidden md:flex flex-1 items-center justify-center">
            <div
              ...
              style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)", boxShadow: "inset 0 1px 4px rgba(0,0,0,0.3)" }}
            >
              {NAV_ITEMS.map(({ icon: Icon, label, active, color, bg }) => (
                <button
                  key={label}
                  className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[11px] font-semibold transition-all duration-150 whitespace-nowrap ${
                    active
                      ...
                      : `${color || "text-white/40"} hover:text-white/80`
                  }`}
                  style={active ? { background: "rgba(255,255,255,0.10)", boxShadow: "0 1px 3px rgba(0,0,0,0.4)" } : {}}
                >
                  <Icon className="h-3 w-3 shrink-0" />
                  {label}
                  {(color) && <ChevronDown className="h-2.5 w-2.5 opacity-50" />}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="flex flex-col items-center justify-center pt-20 px-6 text-center">
        ...
        <h1 className="text-5xl font-bold tracking-tight mb-4" style={{ color: "hsl(220 20% 92%)" }}>
          Free online tools for
        </h1>
        <h1 className="text-5xl font-bold tracking-tight mb-6"
          style={{ background: "linear-gradient(135deg, #60a5fa, #a78bfa, #67e8f9)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
          SEO, creators &amp; developers
        </h1>
        <p style={{ color: "rgba(255,255,255,0.45)", maxWidth: 520, lineHeight: 1.6 }}>
        ...
        <div className="flex gap-3 mt-8">
          <button
            ...
            style={{ background: "linear-gradient(135deg, #7c3aed, #9333ea)", boxShadow: "0 0 20px rgba(139,92,246,0.35)" }}
          >
            Browse All Tools →
          </button>
          <button
        ...
        <div className="mt-16 flex flex-col items-center gap-3">
          <p className="text-xs font-medium" style={{ color: "rgba(255,255,255,0.3)" }}>ACTIVE THEME</p>
          <div className="flex items-center gap-2 px-4 py-2 rounded-full border"
            style={{ borderColor: "rgba(99,102,241,0.3)", background: "rgba(99,102,241,0.1)" }}>
            <Moon className="h-4 w-4 text-indigo-400" />
            <span className="text-sm font-semibold text-indigo-300">Dark Mode</span>
          </div>
        </div>
      </div>
    </div>
